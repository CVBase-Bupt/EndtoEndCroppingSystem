...
class SaliencyUnet(object):
    ...
    def __init__(self, unet_model=None, gamma=3.0, pooling_regions=7, num_rois=1, theta=0.01, stage='train'):
    ...
    def ShareLayer(self, input_tensor=None):
        ...
        conv4 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block4_conv1')(
            pool3)
        conv4 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block4_conv2')(
            conv4)
        ...
        conv5 = Conv2D(1024, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block5_conv1')(pool4)
        conv5 = Conv2D(1024, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block5_conv2')(conv5)

        return [conv1, conv2, conv3, conv4, conv5]

    def UnetLayer(self, X):  # (conv1,conv2,conv3,drop4,drop5)

        up6 = Conv2D(512, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                     name='unet_block6_conv1') \
        ...
        conv8 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block8_conv2')(
            merge8)
        conv8 = Conv2D(128, 3, activation='relu', padding='same', kernel_initializer='he_normal',
                       name='unet_block8_conv3')(
            conv8)
    ...
    def BuildSaliencyModel(self):
        ...
        shared_layer = self.ShareLayer(inputs)
        unet_layer = self.UnetLayer(shared_layer)
        model_unet = Model(inputs, unet_layer)
        if self.unet_model is not None:
            model_unet.load_weights(self.unet_model)
        return model_unet

    def BuildModel(self):

        model_unet = self.BuildSaliencyModel()
        if self.unet_model is not None:
            model_unet.load_weights(self.unet_model)
        saliency_input = model_unet.get_layer('segmentation').output
        feature_input = model_unet.get_layer('unet_block5_conv2').output
        ae_layers = self.AELayer(feature_input, saliency_input, self.stage)
        model_total = Model(model_unet.inputs, ae_layers)
        return model_total